...
int main(int argc, char** argv) {
  ...
  // Used to ensure the number of particles in each class more than a certain
  // number
  ...
  read_generator(inputfile, num_particles, min_radius, max_radius,
                 num_particles_class, num_classes, alpha, beta);

  //! to construct the object of type ParticleGenerator and wrap it in
  //! grain_generator using args&& num_particles, max_radius, min_radius etc.
  // std::shared_ptr<ParticleGenerator<Dim>> grain_generator =
  // std::make_shared<RandomParticleGenerator<Dim>>(num_particles,
  // min_radius,max_radius);

  // read_generator(inputfile,num_particles,min_radius,max_radius,alpha,beta);
  // std::shared_ptr<ParticleGenerator<Dim>> grain_generator =
  // std::make_shared<AcceptRejectGenerator<Dim>>(num_particles,min_radius,max_radius,alpha,beta);

  std::shared_ptr<ParticleGenerator<Dim>> grain_generator =
      std::make_shared<BetaParticleGenerator<Dim>>(
          num_particles, min_radius, max_radius, num_particles_class,
          num_classes, alpha, beta);
